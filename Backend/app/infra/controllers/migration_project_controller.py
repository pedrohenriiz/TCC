...
from interfaces.schemas.migration_project_schema import (
    ...
    MigrationProjectRead  # ← ADICIONAR
)

migration_project_router = APIRouter(prefix="/migration-projects", tags=["Migration Projects"])

@migration_project_router.post("/", response_model=MigrationProjectRead)  # ← ADICIONAR
...
@migration_project_router.get("/")  # ← ADICIONAR
...
def show_migration_project(id: int, db: Session = Depends(database)):
    print("Por favor estou aqui")
    service = MigrationProjectService(db)
    data = service.show_migration_project(id)

    if not data:
        raise HTTPException(status_code=404, detail="Projeto de migração não encontrado!")
    
    return data

@migration_project_router.patch("/{id}", response_model=MigrationProjectRead)  # ← ADICIONAR
